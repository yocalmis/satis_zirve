...
<?php if ($data["teklif_getir_duzenle"]): foreach ($data["teklif_getir_duzenle"] as $dizi_fat): ?>
		...
		<?php if ($data['side_menu'] == "Teklif Ayarları") {echo $data["tek_tur"];}?>
	...
	<input id="fat_turu" name="fat_turu" value="<?php echo $dizi_fat["fatura_turu"]; ?>" type="hidden" placeholder="Fatura Türü" class="material-container__input" />
...
echo $parca[1] . "/" . $parca[2] . "/" . $parca[0];?>"
	...
	<input id="irs_durum" type="hidden" value="<?php echo $dizi_fat["irsaliye_durum"]; ?>" name="irs_durum" />
				...
				<input type="submit" value="Teklifi Güncelle" class="flex-right">
			</div>
