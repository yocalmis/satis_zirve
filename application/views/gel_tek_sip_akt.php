...
<body>
<?php if ($data["teklif_getir_duzenle"]): foreach ($data["teklif_getir_duzenle"] as $dizi_fat): ?>
		<form action="<?php echo base_url(); ?>yonetim/tek_siparis_al" method="post" >
	...
	<input id="fat_turu" name="teklif_id" value="<?php echo $dizi_fat["id"]; ?>" type="hidden" placeholder="Fatura Türü" class="material-container__input" />
...
echo $parca[1] . "/" . $parca[2] . "/" . $parca[0];?>"
	...
	<?php $n = 0;if ($data["teklif_item_getir_duzenle"]): foreach ($data["teklif_item_getir_duzenle"] as $dizi): ?>
				...
				<div class="fatura_row__genel-veriler">
					...
					<input id="qt" type="number" name="qty_<?php echo $n + 1; ?>" value="<?php echo $dizi["adet"]-$dizi["aktarim"]; ?>"  required min="0" placeholder="Miktar" class="material-container__input urunInput" />
