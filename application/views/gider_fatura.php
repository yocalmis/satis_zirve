...
<body>
	...
	<?php if ($data['side_menu'] == "Gider Fatura Ayarları") {echo '<input id="fat_turu" name="fat_turu" value="Gider" type="hidden" placeholder="Fatura Türü" class="material-container__input" />';}?>
			...
			<!--	<select id="mus" name="mus" placeholder="Müşteri" class="material-container__input">
			...
			<?php if ($data["urun_getir"]): foreach ($data["urun_getir"] as $dizi): ?>
				...
				<div class="fatura_row__genel-veriler">
					...
					<input id="qt" type="number" name="qty_1" readonly value="1" required min="0" placeholder="Miktar" class="material-container__input urunInput" />
...
$('select[name=kasa]').css("visibility","hidden");
 $('select[name=kasa]').prop('required',false);
			...
			<select id="item"  name="kasa"  placeholder="Ürün/Hizmet #1" required class="material-container__input" >
