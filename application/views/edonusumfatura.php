...
<body>
	...
	<form action="<?php echo base_url(); ?>yonetim/fatura_aledonusum" method="post" >
...
<label id="top_box2" for="vehicle1">  </label><br>
	...
	<?php if ($data['side_menu'] == "Alış Fatura Ayarları") {echo '<input id="fat_turu" name="fat_turu" value="Alış" type="hidden" placeholder="Sipariş Türü" class="material-container__input" />';}?>
				...
				<select id="mus" name="mus" required="required" placeholder="Müşteri" class="material-container__input">
					<option selected value="<?php echo $data["cari_id"]; ?>"><?php echo $data["cari_ad"]; ?></option>
			</select>
				<!--	<input id="mus" name="mus" type="text" required placeholder="Müşteri" class="material-container__input" />-->
					...
					<input  id="no" name="no" value="" oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);"
    type = "number"
    maxlength = "6" required placeholder="Sipariş No" class="material-container__input" />
			...
			<div class="material-container__row">
				<input id="ack" name="ack" type="text"  required placeholder="Açıklama" value="" style="width: 100%" class="material-container__input" />
...
<div class="material-container__row fatura_row item">
			...
			<option selected value="<?php echo $data["ygdstokid"][$n]; ?>"><?php echo $data["ygdadi"][$n]; ?></option>
				
				
				
			</select>
			<input  id="edon_id_" name="edon_id_<?php echo $n + 1; ?>" type="hidden" value="<?php echo $data["edon_id"][$n]; ?>" required placeholder="ygditemid" class="material-container__input" />
			<input  id="ygditemid" name="ygditemid_<?php echo $n + 1; ?>" type="hidden" value="<?php echo $data["ygditemid"][$n]; ?>" required placeholder="ygditemid" class="material-container__input" />
					<input  id="des" type="text" name="des_<?php echo $n + 1; ?>" value="<?php echo $dizi[0]["aciklama"]; ?>"   placeholder="Açıklama" class="material-container__input" />
		...
		<?php if($data["ygvkn"][$n]!=""){ ?>	
<input id="ekvkn" type="number" value="<?php echo $data["ygvkn"][$n]; ?>" name="ekvkn_<?php echo $n + 1; ?>" min="1111111111" max="9999999999" required  placeholder="Vergi Kimlik no" class="material-container__input" />			
			<?php } ?>			
				</div>						
				
				
				<div class="fatura_row__genel-veriler">
					<input id="pr" step="0.01" type="number" onkeyup="changeprc(<?php echo $n + 1; ?>);" name="prc_<?php echo $n + 1; ?>" value="<?php echo $dizi[0]["birim_fiyat"]; ?>"  required min="0" placeholder="Fiyat" class="material-container__input urunInput" />
				<input id="prh" type="hidden" name="prch_<?php echo $n + 1; ?>" value="<?php echo $dizi[0]["birim_fiyat"]; ?>"  required  placeholder="Fiyath" class="material-container__input urunInput" />	
					<input id="qt" type="number" name="qty_<?php echo $n + 1; ?>" value="" onkeyup	="myFunction3(<?php echo $n + 1; ?>)"  required min="1" max="1"  placeholder="Miktar" class="material-container__input urunInput" />
					<input id="dis" type="number" name="discount_<?php echo $n + 1; ?>" value="<?php echo $dizi[0]["indirim"]; ?>"  required min="0" placeholder="İndirim" class="material-container__input urunInput" />
		

					<input id="tx" type="number" name="tax_<?php echo $n + 1; ?>" value="<?php echo $dizi[0]["vergi"]; ?>" onkeyup	="myFunction2()" min="0" max="18"  placeholder="Vergi" class="material-container__input urunInput" />
					<script>
				...
				{
				 var qt = $("input[name*='qty_"+id+"']").val(); 
       			 if(qt!=1) {  $("input[name*='qty_"+id+"']").val(1); }
					...
					<span> 	<!--<span id="tot" class="totalSpan total_<?php echo $n + 1; ?>"><?php echo $dizi["tutar"] . " " . $this->session->userdata('para_birim'); ?></span>
					</span>-->
					<input id="tot_hidden" onkeyup="changetot(<?php echo $n + 1; ?>);"  placeholder="Toplam" class="material-container__input urunInput" type="text" name="total_<?php echo $n + 1; ?>"/>
				<!--	<button id="tot_hidden" onclick="hesaptotal(<?php echo $n + 1; ?>);" placeholder="Hesapla" class="material-container__input urunInput" type="text" name="total_<?php echo $n + 1; ?>"/>
					-->
					</div>
		...
		<select id="odeme"  name="odeme" placeholder="Ödeme Şekli" class="material-container__input" >
			...
			<div class="faturaSonuc">
				...
				<div class="faturaSonuc__row">
					...
					<span class="faturaSonuc__number ara_toplam"></span>
				</div>
				<div class="faturaSonuc__row">
					<span class="faturaSonuc__title">İndirim </span>
					<span class="faturaSonuc__number indirim"></span>
				</div>
				<div class="faturaSonuc__row">
					<span class="faturaSonuc__title">Vergi </span>
					<span class="faturaSonuc__number vergi"></span>
				</div>
				<div class="faturaSonuc__row">
					<span class="faturaSonuc__title">Toplam </span>
					<span class="faturaSonuc__number toplam"></span>
				</div>

				<input id="ara_toplam" type="hidden" value="" name="ara_toplam" />
				<input id="indirim" type="hidden" value="" name="indirim" />
				<input id="vergi" type="hidden"  value="" name="vergi" />
				<input id="toplam" type="hidden"  value="" name="toplam" >
				<input id="top_alan" type="hidden"  value="<?php echo count($data["fatura_item_getir_duzenle"]); ?>" name="top_alan" />


	<input id="irs_durum" type="hidden" value="" name="irs_durum" />
...
function changetot(x) {
	
	    var val = $("input[name*='total_"+x+"']").val();	
		
        var oran = $("input[name*='tax_"+x+"']").val(); 
		
        var prc = val / (1 + (oran/100));  		
       
        prc = Number(prc);
		$("input[name*='prc_"+x+"']").val(prc.toFixed(2));	
		$("input[name*='prch_"+x+"']").val(prc);
		$("input[name*='qty_"+x+"']").val(1);		
}
