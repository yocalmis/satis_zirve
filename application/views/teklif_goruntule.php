<?php 
 ...
 $this->load->view('header_ozel.php');?>
...
<body> <?php if ($data["teklif_getir_duzenle"]): foreach ($data["teklif_getir_duzenle"] as $dizi_fat):
        $irs_durum = $dizi_fat["irsaliye_durum"];
        $fat_id    = $dizi_fat["id"];
        ...
        if ($data['fat_turu'] == "AlınanTeklif") {
            $form_link     = base_url() . 'yonetim/gel_tek_sip_akt/' . $fat_id;
            $heading_title = 'Alınan Teklif';
            if ($aktarim == 1) {
                $submit_button_value = 'SİPARİŞE AKTAR';
                $hidden_input_name   = 'siparis';
                $hidden_input_value  = 'alinan';
            }
        }
        if ($data['fat_turu'] == "VerilenTeklif") {
            $form_link     = base_url() . 'yonetim/gid_tek_sip_akt/' . $fat_id;
            $heading_title = 'Verilen Teklif';
            if ($aktarim == 1) {
                $submit_button_value = 'SİPARİŞE AKTAR';
                $hidden_input_name   = 'siparis';
                $hidden_input_value  = 'verilen';
            }
			...
			<form action="<?php echo $form_link; ?>" method="post">
				<h2 class="material-container__heading">
					<span class="material-container__title"><?php echo $heading_title; ?></span>
						<input type="submit" value="<?php echo $submit_button_value; ?>"/>
						<input type="hidden" name="<?php echo $hidden_input_name ?>"
     ...
     <?php
        ...
        ?>
				...
				<form action="<?php echo base_url(); ?>yonetim/teklif_guncelle" method="post" >

					<div class="material-container__row musteri_bilgi">

						<input id="fat_turu" name="fat_turu" value="Teklif" type="hidden" placeholder="Fatura Türü" class="material-container__input" />
						...
						<input  id="seri" name="seri" type="text"  readonly="true" value="<?php $fat_id = $dizi_fat["id"];
        echo $dizi_fat["seri_no"];?>" required placeholder="Seri No" class="material-container__input" />
						<input  id="no" name="no" type="text"  readonly="true" required value="<?php echo $dizi_fat["fatura_no"]; ?>" placeholder="Fatura No" class="material-container__input" />
						<input
						id="duz_ta"
        ...
        echo $parca[1] . "/" . $parca[2] . "/" . $parca[0];?>"
					...
					<div class="material-container__row">
						<input id="ack" name="ack" type="text"  readonly="true" required placeholder="Açıklama" value="<?php echo $dizi_fat["aciklama"]; ?>" style="width: 100%" class="material-container__input" />

					</div>
					<?php $n = 0;if ($data["teklif_item_getir_duzenle"]): foreach ($data["teklif_item_getir_duzenle"] as $dizi): ?>
						<div class="material-container__row fatura_row item">
							<div class="fatura_row__genel-bilgi">
								...
								<select id="item"  name="item_<?php echo $n + 1; ?>"  readonly="true" required placeholder="Ürün/Hizmet #1" class="material-container__input" >

									<option selected value="<?php echo $dizi["hizmet_urun_id"]; ?>"><?php echo $data["urun_ad"][$n]; ?></option>
								</select>


								<input  id="des"  readonly="true" type="text" name="des_<?php echo $n + 1; ?>" value="<?php echo $dizi["aciklama"]; ?>"   placeholder="Açıklama" class="material-container__input" />

							</div>
							<div class="fatura_row__genel-veriler">
								<input id="pr" type="number"  readonly="true" name="prc_<?php echo $n + 1; ?>" value="<?php echo $dizi["birim_fiyat"]; ?>"  required min="0" placeholder="Fiyat" class="material-container__input urunInput" />
								...
								<input id="tot_hidden"  readonly="true" type="hidden" value="<?php echo $dizi["tutar"]; ?>" name="total_<?php echo $n + 1; ?>"/>
							</div>
				...
				<div class="faturaSonuc">
					...
					<input id="top_alan" type="hidden"  value="<?php echo count($data["teklif_item_getir_duzenle"]); ?>" name="top_alan" />
...
<?php $n = 1;if ($data["fatura_not_getir"]): foreach ($data["fatura_not_getir"] as $dizi):
        ...
        echo $n . " : " . $dizi["tarih"] . " - " . $dizi["aciklama"] . " - <a href='" . base_url() . "yonetim/fat_not_sil/teklif/" . $dizi["fat_id"] . "/" . $dizi["id"] . "'>Sil</a><br>";
...
<form method="post" action="<?php echo base_url(); ?>yonetim/fatura_not_al" class="input_form">
	...
	<input type="hidden" name="fat_turu"  value="teklif" class="task_input">
