        ...
        $iade_fat  = $dizi_fat["iade_fat"];?>
					...
					<form action="<?php echo base_url(); ?>yonetim/iade/<?php echo $dizi_fat["id"]; ?>" method="post" >
						...
						<?php if ($data['side_menu'] == "Satış Fatura Ayarları"): ?>
							<h2 class="material-container__heading">
								<span class="material-container__title">
									<?php if ($iade_fat != 0): ?>Satış İade Faturası (İade Edilen Fatura)
									<?php else: ?>Satış Faturası
							<?php endif?>
					</span>
					<?php if ($dizi_fat["iade_fat"] == 0): ?>
						<div>
							<!--<input type="submit" value="İADE AL"/>-->
							<input type="hidden" name="iade" value="iade_al" value="NOTLAR"/>
						</div>
					<?php endif?>
				</h2>
			<?php endif?>
			...
			<?php if ($data['side_menu'] == "Alış Fatura Ayarları"): ?>
				<h2 class="material-container__heading">
					<span class="material-container__title">
						<?php if ($iade_fat != 0): ?>
							Alış İade Faturası (İadesi Yapılan Fatura
							<a href="<?php echo base_url() . 'yonetim/fatura_goruntule/' . $iade_fat ?>">Görüntüle</a>)
							<?php else: ?>
								Alış Faturası
							<?php endif?>
						</span>
						<?php if ($dizi_fat["iade_fat"] == 0): ?>
							<div>
								...
								<input type="hidden" name="iade" value="iade_et" value="İADE AL"/>
							</div>
						<?php endif?>
					</h2>
				<?php endif?>
			...
			<form action="<?php echo base_url(); ?>yonetim/fatura_guncelle" method="post" >
				...
				<div class="material-container__row musteri_bilgi">
					...
					<select id="mus" name="mus"   readonly="true" required placeholder="Müşteri" class="material-container__input">
...
echo $parca[1] . "/" . $parca[2] . "/" . $parca[0];?>"
				...
				<?php $n = 0;if ($data["fatura_item_getir_duzenle"]): foreach ($data["fatura_item_getir_duzenle"] as $dizi): ?>
					<div class="material-container__row fatura_row item">
						...
						<div class="fatura_row__genel-veriler">
							...
							<input id="qt" type="number"   readonly="true" name="qty_<?php echo $n + 1; ?>" value="<?php echo $dizi["adet"]; ?>"  required min="0" placeholder="Miktar" class="material-container__input urunInput" />
							<input id="dis" type="number"   readonly="true" name="discount_<?php echo $n + 1; ?>" value="<?php echo $dizi["indirim"]; ?>"  required min="0" placeholder="İndirim" class="material-container__input urunInput" />
							<input id="tx" type="number"   readonly="true" name="tax_<?php echo $n + 1; ?>" value="<?php echo $dizi["vergi"]; ?>"  min="0" placeholder="Vergi " class="material-container__input urunInput" />
	...
	<div id="tabs">
							<ul>
								<!--<li><a href="#tabs-1">PARASAL İŞLEMLER</a></li>
								<li><a href="#tabs-2">İRSALİYELER</a></li>-->
								<li><a href="#tabs-3">NOTLAR</a></li>
							</ul>
							<!--<div id="tabs-1">
...
$n                 = 0;if ($data["fatura_odeme_getir"]): foreach ($data["fatura_odeme_getir"] as $dizi_odeme): ?>
					...
					<?php	if ($dizi_odeme["islem_turu"] == 3) {

        ?>

									<?php echo $n = $n + 1; ?> Tarih: <?php echo $dizi_odeme["tarih"]; ?>  Tutar: <?php echo $dizi_odeme["tutar"] . " " . $this->session->userdata('para_birim');
        $total                = $total + $dizi_odeme["tutar"]; ?> Toplam <?php echo $tur; ?>: <?php echo $total . " " . $this->session->userdata('para_birim')."<br>";
    }
...
?>
    ...
    ?>
										...
										<?php $total = 0;
    $n                     = 0;if ($data["irsaliyeler"]): foreach ($data["irsaliyeler"] as $dizi_irs): ?>
        ...
        echo $n . " : " . $dizi["tarih"] . " - " . $dizi["aciklama"] . " - <a href='" . base_url() . "yonetim/fat_not_sil/fatura/" . $dizi["fat_id"] . "/" . $dizi["id"] . "'>Sil</a><br>";
        ...
        $n = $n + 1;endforeach;endif;?>
							...
							<form method="post" action="<?php echo base_url(); ?>yonetim/fatura_not_al" class="input_form">
								...
								<input type="hidden" name="fat_turu"  value="fatura" class="task_input">
