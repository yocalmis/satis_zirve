	...
	protected function db_insert($state_info)
	{
		...
		if($validation_result->success)
		{
			...
			if($this->callback_insert === null)
			{
				...
				if($this->callback_after_insert_log !== null)
				{
					$callback_return = call_user_func($this->callback_after_insert_log, $post_data, $insert_primary_key);
	...
	protected function db_update($state_info)
	{
		...
		if($validation_result->success)
		{
			...
			if($this->callback_update === null)
			{
				...
				if($this->callback_after_update_log !== null)
				{
					$callback_return = call_user_func($this->callback_after_update_log, $post_data, $primary_key);
		...
		if($this->callback_delete === null)
		{
			...
			if($this->callback_after_delete !== null)
			{
				...
				if($callback_return === false)
				{
					...
					if($this->callback_after_delete_log !== null)
			{
				$callback_return = call_user_func($this->callback_after_delete_log, $primary_key_value);
	...
	protected function get_text_input($field_info,$value)
	{
		if($field_info->extras == 'text_editor')
		{
			...
			switch ($editor) {
				...
				case 'medium-editor':
					$this->set_css($this->default_texteditor_path.'/medium-editor/medium-editor.min.css');
					$this->set_css($this->default_texteditor_path.'/medium-editor/themes/default.min.css');
					$this->set_js_lib($this->default_texteditor_path.'/medium-editor/medium-editor.min.js');
					$this->set_js_config($this->default_texteditor_path.'/medium-editor/medium-editor-config.js');
				break;
...
class Grocery_CRUD extends grocery_CRUD_States
{
	...
	public function callback_after_insert($callback = null)
	{
		...
		public function callback_after_insert_log($callback = null)
	{
		$this->callback_after_insert_log = $callback;
	...
	public function callback_after_update_log($callback = null)
	{
		$this->callback_after_update_log = $callback;
	...
	public function callback_after_delete($callback = null)
	{
		...
		public function callback_after_delete_log($callback = null)
	{
		$this->callback_after_delete_log = $callback;
